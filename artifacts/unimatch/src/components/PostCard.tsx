...
function Avatar({ author }: { author?: SocialPost["author"] | null }) {
  if (!author) {
    return <div className="h-10 w-10 rounded-2xl bg-muted" />;
  }
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-xl border border-border px-4 py-2 text-sm font-black hover:bg-muted"
            >
              <X className="h-4 w-4" />
              Cancel
            </button>
        ...
        <>
          ...
          {post.url && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-4 inline-block rounded-xl border border-border px-4 py-2 text-sm font-black text-primary hover:bg-muted"
            >
              Open link
            </a>
          )}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black transition hover:bg-muted ${
                post.likedByMe ? "text-red-600" : "text-foreground"
              }`}
            >
            ...
            <div className="mt-4 space-y-3 rounded-2xl border border-border bg-background p-4">
              ...
              {(commentsQuery.data ?? []).map((c) => (
                <div key={c.id} className="flex gap-3 rounded-xl bg-card p-3">
                  ...
                  <div>
                    <Link href={c.author?.username ? `/profile/${c.author.username}` : "/profile"} className="text-sm font-black hover:underline">
